...
; 任务控制块TCB是我们自己发明的
; 遍历整个tcb_chain找到最后一个TCB，next指向我们传入的tcb
...
;-------------------------------------------------------------------------------
;基础概念：
;* LDT是任务自有的段描述符表(LDTR寄存器指向它)
;段选择子中的第2位(从0开始)是表指示器，TI=1代表从当前任务的LDT中加载描述符
;* TSS是任务状态段(TR寄存器指向它)
;保存切换任务时的上下文（寄存器之类的）
;* 除了段描述符里的段的特权级DPL，段选择子里的任务的特权级RPL（请求特权级），
;代码段的特权级叫CPL（就是代码段的DPL）
;EFLAGS寄存器中IOPL位是任务自己的I/O特权级
;代码段的段描述符的TYPE字段里的C位（依从位），
;当C=0时只可共同级程序运行，C=1，可以供更低级（等级数字更高）的程序调用，但禁止使用低级程序(等级数字更高)的数据
;即使调用更高级(等级数字更低)的依从的代码段，程序的特权级不变
;简单说，就是实现了用户态可以调用核心态提供的函数，访问核心态的数据，但核心态的函数不能使用用户态的数据，
;* 门(Gate)描述符，描述可执行的代码，可以jmp far 门描述符，调用门里也有DPL，调用门选择符也有RPL
;特权级检查：
;https://www.cnblogs.com/chenwb89/p/operating_system_004.html
;call检查 当前CPL<=调用门描述符DPL，RPL<=调用门描述符DPL，当前CPL>=目的代码段描述符DPL
;jmp检查 当前CPL<=调用门描述符DPL，RPL<=调用门描述符DPL，
; 如果目的代码段是依从话，CPL>=目的代码段的DPL，
; 而如果目的代码段是非依从的话，CPL=目的代码段的DPL。
start:
  ...
  .b3:
         ...
         ltr [ecx+0x18]                     ;加载任务状态段 TSS 16位
         lldt [ecx+0x10]                    ;加载LDT 16位
      
         mov eax,[ecx+0x44]
         mov ds,eax                         ;切换到用户程序头部段 

         ;因为不允许从0特权级跳转到3特权级，所以现在要伪装成从控制门返回。
         ;栈布局详见图：特权级间控制转移时的栈切换
