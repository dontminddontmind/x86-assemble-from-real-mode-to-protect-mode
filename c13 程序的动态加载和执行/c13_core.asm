...
; vstart 程序中的[ebx]和标号之类的地址会加上vstart的值
; align 汇编地址对齐方式，如16、32等
...
put_string:                                 ;显示0终止的字符串并移动光标 
  ...
  .exit:
       ...
       ; 实模式可以push段地址+偏移地址，再retf，实现段间跳转
...
put_char:                                   ;在当前光标处显示一个字符,并推进
         ...
         ;以下取当前光标位置
         ;(本书用的是VGA显示标准）
         ;光标位置在两个光标寄存器中，合成一个16位的值
         ;这个值，0代表第0行第0列，80代表第1行第0列，因为标准VGA模式是25行*80列
         ;光标寄存器可读可写，因为显卡不移动光标，需要我们设置
         ;索引寄存器：
         ;为了不占用过多I/O空间，很多寄存器只能通过索引寄存器间接访问
         ;0x3d4端口，out写入值，用于指定寄存器（光标寄存器的索引分别是0x0e和0x0f)
         ;0x3d5端口，读写上面指定的寄存器
         mov dx,0x3d4
         mov al,0x0e
         out dx,al
  ...
  .roll_screen:
        ...
        ;将屏幕的内容都上抬一行（将后1920个字符重头输出一遍）
         ...
         mov ecx,1920
         ;rep 重复执行指令ecx次
         ;movsb movsw movsd : 从源地址(esi)向目的地址(edi)传送数据，b是字节，w是字，d是双字
         ;DF=0，每次操作esi和edi递增，反之同理
         rep movsd

         mov bx,3840                        ;清除屏幕最底一行
         mov ecx,80                         ;32位程序应该使用ECX
  .cls:
         mov word[es:bx],0x0720             ;显示一个空格
         add bx,2
         loop .cls
...
start:
         ...
         mov eax,0x80000002
         cpuid ;返回处理器的标识和特性信息
         ;eax指定信息，为0返回最大可支持功能号到eax，并返回处理器供应商的字符串到ebx、edx、ecx寄存器
