         ...
         ;创建1#描述符，这是一个数据段，对应0~4GB的线性地址空间，可以访问所有地方
  ...
  flush:                                     
       ; 我猜：
       ; 按理来说16位寄存器操作应该用ax而不是eax
       ; 但是如果用ax，那么因为在32位模式下，默认32位操作，如果用16位操作需要在机器码前加0x66的前缀
       ; 但是真实机器对这段代码做了优化，对段寄存器赋值不用0x66前缀
       ; 但编译器分不出来，还是会自己加上前缀
       ; 于是这里用eax，编译器不会为32位操作加前缀
         mov eax,0x0018                      
         mov ds,eax
       
       ; 给段寄存器赋值时会检查段描述符和寄存器类型是否匹配
       ; 比如不可读代码段就不能赋值给ds
       ; p位为0（段不在内存中），cpu就中断去加载段
